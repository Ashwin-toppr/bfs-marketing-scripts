...
const isExist = () => {
  $.ajax({
    type: "POST",
    url: `${PROD_BASE_URL}/api/V1/userDetail/existByEmailOrMobile?timezone=${timeZone}&regionId=${country}&courseType=MATH&brandId=byju&timestamp=${new Date().getTime()}`,
    cache: false,
...
const getOtp = (callback, isResend) => {
  const url = isUserExist
    ? `${PROD_BASE_URL}/api/V1/users/sendStudentVerificationCode?timezone=${timeZone}&regionId=${country}&courseType=MATH&brandId=byju&timestamp=${new Date().getTime()}`
    : `${PROD_BASE_URL}/api/V1/otp/generate?regionId=${country}&timezone=${timeZone}&courseType=MATH&brandId=byju&timestamp=${new Date().getTime()}`;
...
$(".otp-input-box").on("input", (e) => {
  ...
  if (e.target.value.length >= 4) {
    ...
    const url = isUserExist
      ? `${PROD_BASE_URL}/api/V1/users/authenticateVerificationCode?timezone=${timeZone}&_vercel_no_cache=1&regionId=${country}&courseType=MATH&brandId=byju&timestamp=${new Date().getTime()}`
      : `${PROD_BASE_URL}/api/V1/otp/verify?timezone=${timeZone}&regionId=${country}&brandId=byju&courseType=MATH&timestamp=${new Date().getTime()}`;
    $.ajax({
      type: "POST",
      url: url,
      cache: false,
...
const handleRegisterUser = () => {
  $.ajax({
    type: "POST",
    url: `${PROD_BASE_URL}/api/V1/trial/users/minimalFieldRegister?timezone=${timeZone}&courseType=MATH&isMobilePlatform=false&brandId=byju&timestamp=${new Date().getTime()}`,
    cache: false,
    data: {
...
const handleMecall = () => {
  $.ajax({
    type: "GET",
    url: `${PROD_BASE_URL}/api/V1/userDetail/me?timezone=${timeZone}&courseType=MATH&brandId=byju&timestamp=${new Date().getTime()}`,
    cache: false,
    headers: {
