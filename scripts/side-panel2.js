...
const handleGetSlots = () => {
  ...
  $.ajax({
    type: "GET",
    url: `${STAG_BASE_URL}/api/V1/trial/slots/get?countryCode=${country}&grade=${selectedGrade}&timezone=${timeZone}&courseType=${selectedSubj.toUpperCase()}${selectedSubj.includes('music') ? '&courseSubType='+courseSubType : ''}`,
    cache: false,
    headers: {
      authorization: `Bearer ${token}`,
    },
...
const handleBookSlot = () => {
  ...
  $.ajax({
    type: "POST",
    url: `${STAG_BASE_URL}/api/V1/trial/slots/book?timezone=${timeZone}&regionId=${country}&courseType=${selectedSubj.toUpperCase()}&${selectedSubj.includes('music') ? `courseSubType=${courseSubType}`:''}`,
    cache: false,
    data: {
      countryCode: country,
      slot: {
...
const getTimeZonesEmbedded = (TZList = timezonesList) => {
  ...
  TZList.map((zone) => {
    if(isMweb){
      item = `<div class="mweb-timezone-item"><p  class="paragraph-53 events-none">${zone.zoneName}</p><p class="paragraph-54 events-none">(${GMTOffset(zone.gmtOffset)})</p><div  class="mweb-checked-image events-none ${
    ...
    $(`${isMweb ? ".mweb-timezones-container" : ".timezones-container"}`).append(
      item
    );
  });
