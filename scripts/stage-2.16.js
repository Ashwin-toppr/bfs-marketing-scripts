...
const isExist = () => {
  $.ajax({
    type: "POST",
    url: `${PROD_BASE_URL}/api/V1/userDetail/existByEmailOrMobile?timezone=${timeZone}&regionId=${country}&courseType=${selectedSubj.toUpperCase()}&brandId=byju&timestamp=${new Date().getTime()}`,
    cache: false,
...
const getOtp = (callback, isResend) => {
  const url = isUserExist
    ? `${PROD_BASE_URL}/api/V1/users/sendStudentVerificationCode?timezone=${timeZone}&regionId=${country}&courseType=${selectedSubj.toUpperCase()}&brandId=byju&timestamp=${new Date().getTime()}`
    : `${PROD_BASE_URL}/api/V1/otp/generate?regionId=${country}&timezone=${timeZone}&courseType=${selectedSubj.toUpperCase()}&brandId=byju&timestamp=${new Date().getTime()}`;
...
$(".otp-input-box").on("input", (e) => {
  ...
  if (e.target.value.length >= 4) {
    ...
    const url = isUserExist
      ? `${PROD_BASE_URL}/api/V1/users/authenticateVerificationCode?timezone=${timeZone}&_vercel_no_cache=1&regionId=${country}&courseType=${selectedSubj.toUpperCase()}&brandId=byju&timestamp=${new Date().getTime()}`
      : `${PROD_BASE_URL}/api/V1/otp/verify?timezone=${timeZone}&regionId=${country}&brandId=byju&courseType=${selectedSubj.toUpperCase()}&timestamp=${new Date().getTime()}`;
    $.ajax({
      type: "POST",
      url: url,
      cache: false,
